...
export class GameService {
  ...
  async getGameByTypeAndNumber(type: string, gameNumber: number) {
    const result = await this.gameModel.findOne({ type: type, number: gameNumber }).exec()
  ...
  private async processUpdate(openTask: Task) {
    ...
    const typesForRequest: { type: string, urlParameters: string }[] = [
      ...
      {
        type: "quina",
        urlParameters: "quina/!ut/p/a1/jc69DoIwAATgZ_EJepS2wFgoaUswsojYxXQyTfgbjM9vNS4Oordd8l1yxJGBuNnfw9XfwjL78dmduIikhYFGA0tzSFZ3tG_6FCmP4BxBpaVhWQuA5RRWlUZlxR6w4r89vkTi1_5E3CfRXcUhD6osEAHA32Dr4gtsfFin44Bgdw9WWSwj/dl5/d5/L2dBISEvZ0FBIS9nQSEh/pw/Z7_61L0H0G0J0VSC0AC4GLFAD20G6/res/id=buscaResultado"
      }
    ]
    ...
    const allAsyncUpdateProcess = typesForRequest.map(async (requestType) => {
      ...
      for (let index = lastGame; index > 0; index--) {
        ...
        try {
          const result = await this.gameModel.find({ type: requestType.type.toUpperCase(), number: index }).exec()
  ...
  async search(request: Search) {
    ...
    const results = await this.gameModel.find(this.buildWhere(request))
      .select(this.buildSelect(request))
      .skip(request.page).limit(request.amount)
  ...
  private buildSelect(request: Search): any {
    return request.fields ? request.fields.map(field => {

      if (field.includes("id")) {
        return field
      }
      const splitFields = field.split(".").map(subField => toCamelCase(subField));
  ...
  private buildWhere(request: Search) {
    ...
    if (request.from != null && request.to != null)
      Object.assign(whereObj, { date: { $gte: request.from, $lte: request.to } })
